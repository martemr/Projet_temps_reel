...
void Tasks::Init() {
    ...
    if (err = rt_task_create(&th_WatchDog, "th_WatchDog", 0, PRIORITY_TSTARTROBOT, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::BatteryLevel(void *arg){
    cout << "Start Battery Level" << __PRETTY_FUNCTION__ << endl << flush;
    // Synchronization barrier (waiting that all tasks are starting)
    rt_sem_p(&sem_barrier, TM_INFINITE);
    ...
    while (1) {
        ...
        if(rs==1){
            Message *batteryAnswer;
            rt_mutex_acquire(&mutex_robot, TM_INFINITE);
            batteryAnswer = robot.Write(robot.GetBattery());
            rt_mutex_release(&mutex_robot);

            WriteInQueue(&q_messageToMon, batteryAnswer);
        }
    }
}
